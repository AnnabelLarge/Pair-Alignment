...
assert np.allclose( joint_transit[:,-1,1], cond_transit[:,-1,1] )
...
assert np.allclose(log_beta, joint_transit[:,-1,1])
assert np.allclose(log_beta, cond_transit[:,-1,1])
...
assert np.allclose(np.array(hand_done_anc_marg),
...
assert np.allclose( manual_cond_transit, joint_minus_marg_transit )
...
_, neural_cond_transit, _ = out
neural_cond_transit = neural_cond_transit.sum(axis=-1)

assert np.allclose( neural_cond_transit, joint_minus_marg_transit )
...
assert np.allclose(cond_logprob_transits, different_scoring_mat)
...
out = neural.apply( variables=init_params,
                            ...
                            return_transit_emit = True,
                            method='neg_loglike_in_scan_fn') #(T,B) or (B,L)
_, neural_cond_transit_different_mat, _ = out
neural_cond_transit_different_mat = neural_cond_transit_different_mat.sum(axis=-1)

assert np.allclose( neural_cond_transit_different_mat, joint_minus_marg_transit )
