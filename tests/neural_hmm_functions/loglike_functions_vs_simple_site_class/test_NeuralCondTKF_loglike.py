...
class TestNeuralLoglikeVsPairhmmLoglike(unittest.TestCase):
    def setUp(self):
        ...
        with open(f'{self.req_files_loc}/equl_dist.npy','wb') as g:
            np.save(g, training_dset_emit_counts/training_dset_emit_counts.sum())
        
        with open(f'{self.req_files_loc}/tkf91_params_file.pkl','wb') as g:
            pickle.dump(tkf91_params, g)
            
        with open(f'{self.req_files_loc}/tkf92_params_file.pkl','wb') as g:
        ...
        self.common_config = {'training_dset_emit_counts': training_dset_emit_counts,
                              ...
                              'filenames': {'exch': f'{self.req_files_loc}/LG08_exchangeability_vec.npy',
                                            'equl_dist': f'{self.req_files_loc}/equl_dist.npy'}}
    ...
    def _run_test(self,
        ...
        self.common_config['filenames']['tkf_params_file']= f'{self.req_files_loc}/{indel_model_type}_params_file.pkl'
        ...
        if indel_model_type == 'tkf92':
            ...
            corr_num = lam/mu
            corr_denom = (r + (1-r)*(lam/mu))
            corr = corr_num / corr_denom
            corr_factors = np.log(corr)
    
            if not unique_t_per_samp:
                corr_factors = corr_factors[None]  # shape is (1,); broadcast to (T,B)
    
        elif indel_model_type == 'tkf91':
            shape = (1,) if unique_t_per_samp else (1, 1)
            corr_factors = jnp.zeros(shape)
        ...
        if jit_compiled:
            jitted_apply = jax.jit(pairhmm.apply, 
                                   static_argnames=('method','return_intermeds','return_all_matrices'))
        ...
        neural_config = {'load_all': False,
                         ...
                         'emissions_postproc_config': {'norm_rate_mult': False,
                                                       'emission_alphabet_size': np.array(self.A),
                                                       'training_dset_emit_counts': self.common_config['training_dset_emit_counts']},
                         
                         'transitions_postproc_model_type': None,
                         'transitions_postproc_config': {} }
