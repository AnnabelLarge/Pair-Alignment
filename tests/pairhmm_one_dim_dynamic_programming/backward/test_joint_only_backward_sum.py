...
class TestJointOnlyBackward(unittest.TestCase):
    def setUp(self):
        # fake inputs
    ...
    def test_backward_checksum(self):
        ### pred
        backward_intermeds = joint_only_backward(aligned_inputs = self.fake_aligns,
                                                   ...
                                                   unique_time_per_sample = False) #(L-1, T, C, B)
        
        # checksum against forward function
        fw_sum = joint_only_forward( aligned_inputs = self.fake_aligns,
                                     joint_logprob_emit_at_match = self.joint_logprob_emit_at_match,
                                     logprob_emit_at_indel = self.logprob_emit_at_indel,
                                     joint_logprob_transit = self.joint_logprob_transit,
                                     unique_time_per_sample = False,
                                     return_all_intermeds = False ) #(T, B)
        
        bkw_sum = logsumexp(backward_intermeds[0,...], axis=1) #(T, B)
        
        npt.assert_allclose( fw_sum, bkw_sum )
