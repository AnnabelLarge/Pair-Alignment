...
def main(fold, t_per_samp):
    ...
    def extract_info(sub_df, standard_model):
        for i in range(len(sub_df)):
            ...
            ### tkf params
            path = f'{fold}/{d}/out_arrs/PARAMS_{indel_model}_indel_params.txt'
            ...
            if indel_model == 'tkf91':
                ...
                params['adjusted_lam'].append(lam)
                params['adjusted_mu'].append(mu)
    ...
    # raw lambda, mu, r, mean fragment length, adjusted lam, and adjusted mu 
    # as (6, num_runs) table
    lam = np.concatenate( params['lam'] )
    mu = np.concatenate( params['mu'] )
    r = np.concatenate( params['r'] )
    mean_fragment_len = np.concatenate( params['mean_fragment_len'] )
    adjusted_lam = np.concatenate( params['adjusted_lam'] )
    adjusted_mu = np.concatenate( params['adjusted_mu'] )
    final_mat = np.stack([lam, mu, r, mean_fragment_len, adjusted_lam, adjusted_mu])
    
    indexes = ['lam', 'mu', 'r', 'mean_fragment_len', 'adjusted_lam', 'adjusted_mu']
    
    df = pd.DataFrame(final_mat, 
                      columns=params['run'],
                      index=indexes)
    
    df.to_csv(f'{fold}/BEST_indel_params.tsv', sep='\t')
    
    
    # all variable-length things: class prob, rate multiplier; flat text file, 
