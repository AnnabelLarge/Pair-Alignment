...
del m_idx, b, current_a, current_d_or_gap, l, B, L_align
...
# <eos> only appears twice
assert desc_tok_counts[2] == 2
assert desc_given_current_anc_tok_counts[2,:].sum() == 2
assert desc_given_current_anc_tok_counts[:,2].sum() == 2
assert desc_given_current_anc_tok_counts[2,2] == 2
...
### write files
with open(f'{prefix}_desc-align_counts.npy','wb') as g:
    np.save( g, desc_tok_counts )
    
with open(f'{prefix}_desc-align_given_current_anc_counts.npy','wb') as g:
