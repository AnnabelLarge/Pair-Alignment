...
class MarkovSitesJointPairHMM(ModuleBase):
    ...
    def __call__(self,
        ...
        ######################################################
        def scan_fn(prev_alpha, curr_index):
        ...
        # T, B
        logprob_perSamp_perTime = logsumexp(final_alpha, axis=1)
...
class WithForLoopMarkovSitesJointPairHMM(MarkovSitesJointPairHMM):
    ...
    def __call__(self,
        ...
        alpha = tr + e
        for curr_index in range(2, aligned_inputs.shape[1]):
            prev_state = aligned_inputs[:,curr_index-1,2]
            curr_state = aligned_inputs[:,  curr_index,2]
            anc_toks =   aligned_inputs[:,  curr_index,0]
            desc_toks =  aligned_inputs[:,  curr_index,1]
