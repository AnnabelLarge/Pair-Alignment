...
class MarkovFragSites(ModuleBase):
    """
    ...
    def calculate_all_loglikes(self,
        ...
        if (not unique_time_per_branch) and (t_array.shape[0] > 1):
            joint_logprob_perSamp_maybePerTime = aux_dict['joint_neg_logP']  #(T,B)
        ...
        if self.norm_loss_by == 'desc_len':
            ...
            cond_neg_logP_length_normed = out['cond_neg_logP'] / desc_len
        
        elif self.norm_loss_by == 'align_len':
            joint_neg_logP_length_normed = out['joint_neg_logP'] / align_len
            cond_neg_logP_length_normed = out['cond_neg_logP'] / align_len
...
class MarkovFragSitesLoadAll(MarkovFragSites):
    """
    same as  MarkovFragSites, but load all parameters from files (excluding time, 
