"""
This sets up the recursive scan for SSMs (from Ian)

Basically, this explicitly defines the forward and backwards method
...
### helper functions #
######################
# The associative scan is a product of matrices of the form ((g,h),(0,1)) 
# where g_i=exp(A*Delta)x_i and h_i=B*Delta*x_i
# Since matrices of this form are are closed under multiplication, we can 
# represent all intermediate products in the same way
@jax.remat
...
# The zero-order hold is empirically only really necessary for alpha, 
# since it has a gating effect
@jit
...
def ssm_recursive_scan (x, Acoeff, Bcoeff, Ccoeff, Delta, 
    ...
    # split into chunks
    # This is a bit inefficient, but taking dynamic slices appears to be 
    # worse in terms of GPU memory usage
    ...
    def scan_chunk (carry, chunk):
        ...
        if chunk_size > min_recursion_length and chunk_size % recursive_split == 0:
            # Split inputs into chunks, scan each chunk, and concatenate 
            # results
            # Again, this seems inefficient, but empirically uses less GPU 
            # memory than passing an index range and doing dynamic slicing
...
# wrapper to help define VJP (at the end)
...
### wrapper that helps reshape things
@partial(jit, static_argnames=('min_recursion_length','recursive_split',))
...
###################################################
### The forward and backwards method for ssm_scan #
###   explicitly united here                      #
###################################################
# vjp = vector jacobian product = forward-mode auto differentiation
